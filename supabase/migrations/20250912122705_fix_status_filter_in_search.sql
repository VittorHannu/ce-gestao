...
BEGIN
    ...
    IF p_status_filter IS NOT NULL THEN
        ...
        ELSIF p_status_filter = 'EM_ANDAMENTO' THEN
            query_conditions := query_conditions || ' AND r.status = ' || quote_literal('APROVADO') || ' AND r.planejamento_cronologia_solucao IS NOT NULL AND r.data_conclusao_solucao IS NULL AND r.concluido_sem_data = FALSE';
        ELSIF p_status_filter = 'SEM_TRATATIVA' THEN
            query_conditions := query_conditions || ' AND r.status = ' || quote_literal('APROVADO') || ' AND r.planejamento_cronologia_solucao IS NULL AND r.data_conclusao_solucao IS NULL AND r.concluido_sem_data = FALSE';
        END IF;
    END IF;
