...
-- Recreate the search function with an explicit column list to prevent structural mismatches.
...
BEGIN
    ...
    WITH filtered_relatos AS (
        SELECT 
            -- Explicitly list columns to ensure structure matches RETURNS TABLE
            r.id, 
            r.created_at, 
            r.local_ocorrencia, 
            r.descricao, 
            ...
            r.user_id, -- needed for p_only_mine filter
            r.fts_document, -- needed for search filter
