-- Drop the old function first, since we are changing the return signature
DROP FUNCTION IF EXISTS public.get_relato_details(uuid);

-- Recreates the function to get all necessary details for a single relato view in one query.
-- This version removes the non-existent 'avatar_url' column.
