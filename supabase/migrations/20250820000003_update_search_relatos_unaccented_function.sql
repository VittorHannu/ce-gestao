...
DROP FUNCTION IF EXISTS public.search_relatos_unaccented(text, text, text, date, date);
DROP FUNCTION IF EXISTS public.search_relatos_unaccented(text, text, text, date, date, text);

CREATE OR REPLACE FUNCTION "public"."search_relatos_unaccented"("p_search_term" "text", "p_status_filter" "text", "p_responsible_filter" "text", "p_start_date" "date", "p_end_date" "date", "p_tipo_relato_filter" "text") RETURNS SETOF "public"."relatos"
    LANGUAGE "plpgsql"
    AS $
DECLARE
