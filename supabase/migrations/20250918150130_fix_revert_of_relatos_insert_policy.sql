-- Drop the restrictive policy that was re-introduced by a later migration.
DROP POLICY IF EXISTS "Allow authenticated users to insert their own relatos" ON public.relatos;

-- Re-create the correct, flexible policy that allows authenticated users to insert relatos
