...
export async function getAccidentRecordStats() {
  const { data, error } = await supabase.rpc('get_record_sem_acidentes');

  if (error) {
    console.error('Error fetching accident record stats:', error);
    throw error;
  }
